...
namespace AOC2018
{
    public class Day4
    {
        ...
        {
            var input = @"[1518-05-08 00:12] falls asleep
...
[1518-06-08 00:40] falls asleep";
            ...
            {
                var match = duty.Match(action);
                if (match.Success)
                {
                    id = ReadInt(match, "id");
                    current = GuardFactory.GetGuard(id);
                    continue;
                }
                match = sleep.Match(action);
                if (match.Success)
                {
                    awake = false;
                    sleepingTime = ReadInt(match, "minute");
                    continue;
                }
                match = wakeup.Match(action);
                if (match.Success)
                {
                    awake = true;
                    current.Add(sleepingTime, ReadInt(match, "minute"));
                    continue;
                }
                throw new ArgumentException($"{action} not matched correctly");
            }
            int maxcount = 0;
            Guard target = null;
            foreach (var g in GuardFactory.AllGuards())
            {
                if (g.Count > maxcount)
                {
                    target = g;
            ...
            for (int i = 0; i < 60; i++)
            {
                if (target.Minutes[i] > maxcount)
                {
                    maxcount = target.Minutes[i];
                    maxMinute = i;
                }
            }
            Console.WriteLine($"{maxMinute} * {target.Id}= {maxMinute * target.Id}");
            maxMinute = 0;
            maxcount = 0;
            target = null;
            foreach (var g in GuardFactory.AllGuards())
            {
                for (int i = 0; i < 60; i++)
                {
                    if (g.Minutes[i] > maxcount)
                    {
                        maxcount = g.Minutes[i];
                        target = g;
                        maxMinute = i;
                    }
                }
        ...
        static string datetime = @"\[(?<year>[\d]{4})-(?<month>[\d]{2})-(?<day>[\d]{2}) (?<hour>[\d]{2}):(?<minute>[\d]{2})\]";
        static Regex duty = new Regex(datetime + @" Guard #(?<id>[\d]*) begins shift");
        static Regex sleep = new Regex(datetime + @" falls asleep");
        static Regex wakeup = new Regex(datetime + @" wakes up");
    ...
    internal class GuardFactory
    {
        static Dictionary<int, Guard> guards = new Dictionary<int, Guard>();
        ...
        public static IList<Guard> AllGuards()
        {
