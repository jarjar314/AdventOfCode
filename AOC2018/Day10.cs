...
namespace AOC2018
{
    public class Day10
    {
        ...
        {
            var input = @"position=< 50769, -40375> velocity=<-5,  4>
...
position=<-30175,  10254> velocity=< 3, -1>";
            ...
            foreach (var line in input.Split('\n'))
            {
                ...
                int vx = int.Parse(tokens[4]);
                int vy = int.Parse(tokens[5]);
                lights.Add(new Light() { X = x, Y = y, VX = vx, VY = vy, Id = id++ });
            }
        ...
        //10124
        private static void Print(List<Light> lights, int turn)
        {
            var point = lights.Select(l => l.GetPosition(turn)).ToList();
            var xmin = point[0].x;
            var xmax = point[0].x;
            var ymin = point[0].y;
            var ymax = point[0].y;
            ...
            {
                xmin = Math.Min(xmin, point[i].x);
                ymin = Math.Min(ymin, point[i].y);
                xmax = Math.Max(xmax, point[i].x);
                ymax = Math.Max(ymax, point[i].y);
            }
    ...
    internal class Light
    {
        ...
        public (int x, int y) GetPosition(int turn)
        {
            return (X + VX * turn, Y + VY * turn);
        }
    }
}
