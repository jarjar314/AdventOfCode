...
U 2 (#7a21e3)";
        ...
        private static void part1(string input)
        {
            ...
            var visited = new SortedSet<(int r, int c)>();
            ...
            flood(grid, 0, 0);
            count = grid.Sum(row => row.Count(c => c == '#' || c == 'I'));
        ...
        private static void part2(string input)
        {
            ...
            long count = 0;
            int m = inputs.Length, n = inputs[0].Length;
            var visited = new List<(long r, long c)>();
            long rmin = 0, rmax = 0, cmin = 0, cmax = 0;
            long r = 0, c = 0;
            visited.Add((r, c));
            long path = 0;
            for (int i = 0; i < m; i++)
            {
                var line = inputs[i].Split(new char[] { ' ', '(', ')' }, StringSplitOptions.RemoveEmptyEntries);
                int direction = (line[2][^1] - '0' + 1) % 4;
                long steps = Convert.ToInt64(line[2][1..^1],16);
                path += steps;
                r += steps * dr[direction];
                c += steps * dc[direction];
                visited.Add((r, c));
            }

            for (int i = 1; i < visited.Count; i++)
            {
                count += visited[i].r * visited[i - 1].c - visited[i].c * visited[i - 1].r;
            }
            count = Math.Abs(count) + path;

            Console.WriteLine($"2nd star is {count} => {count /2 + 1}");
        }
