...
namespace AOC2016
{
    public class Day10
    {
        static readonly Regex botInitReader = new Regex(@"^value (?<chip>[0-9]+) goes to bot (?<bot>[0-9]+)");
        static readonly Regex botGivesReader = new Regex(@"^bot (?<bot>[0-9]+) gives low to (?<typeLow>[a-z]+) (?<botLow>[0-9]+) and high to (?<typeHigh>[a-z]+) (?<botHigh>[0-9]+)");
        public static void Main()
        {
            var input = @"value 61 goes to bot 209
...
bot 72 gives low to bot 9 and high to bot 77";
            ...
            Console.WriteLine(FindBot61vs17(input));
            Console.WriteLine(string.Join(" ", outputs[0].MicroChips));
            Console.WriteLine(string.Join(" ", outputs[1].MicroChips));
            Console.WriteLine(string.Join(" ", outputs[2].MicroChips));
        }
        static readonly Dictionary<int, Bot> bots = new();
        static readonly Dictionary<int, Output> outputs = new();
        private static int FindBot61vs17(string input)
        {
            var inputs = input.Split('\n');
            Match m;
            var twoChipsBot = new Queue<Bot>();
            foreach (var line in inputs)
            {
                m = botInitReader.Match(line);
                if (m.Success)
                {
                    var botId = int.Parse(m.Groups["bot"].Value);
                    var chip = int.Parse(m.Groups["chip"].Value);
                    var bot = GetBot(botId);
                    bot.AddChip(chip);
                    if (bot.IsActive())
                        twoChipsBot.Enqueue(bot);
                    continue;
                }
                m = botGivesReader.Match(line);
                if (m.Success)
                {
                    var botId = int.Parse(m.Groups["bot"].Value);
                    var botHighId = int.Parse(m.Groups["botHigh"].Value);
                    var botLowId = int.Parse(m.Groups["botLow"].Value);
                    var typeHigh = m.Groups["typeHigh"].Value;
                    var typeLow = m.Groups["typeLow"].Value;
                    var bot = GetBot(botId);
                    var elementHigh = GetElement(botHighId, typeHigh);
                    var elementLow = GetElement(botLowId, typeLow);
                    bot.TargetHigh = elementHigh;
                    bot.TargetLow = elementLow;
                    continue;
                }
                throw new ArgumentException($"{line} is not supported by regex.");
            }
            var res = -1;
            while (twoChipsBot.Count > 0)
            {
                var bot = twoChipsBot.Dequeue();
                if (bot.MicroChips.Contains(17) && bot.MicroChips.Contains(61))
                    res = bot.Id;
                bot.GiveChip();
                if (bot.TargetLow.IsActive()) twoChipsBot.Enqueue((Bot)bot.TargetLow);
                if (bot.TargetHigh.IsActive()) twoChipsBot.Enqueue((Bot)bot.TargetHigh);
            }
            return res;
        }

        private static IElement GetElement(int elementId, string typeElement)
        {
            return typeElement switch
            {
                "bot" => GetBot(elementId),
                "output" => GetOutput(elementId)
            };
        }
        ...
        private static Output GetOutput(int outputId)
        {
            if (!outputs.TryGetValue(outputId, out var output))
            {
                output = new Output() { Id = outputId };
    ...
    {
        void AddChip(int chipId);
        List<int> MicroChips { get; set; }
        int Id { get; set; }
        bool IsActive();
    }
    ...
    public class Bot : IElement
    {
        public List<int> MicroChips { get; set; }
        public int BotReceivingLowChip { get; set; }
        public int BotReceivingHighChip { get; set; }
        public List<(int low, int high)> History { get; set; }
        public IElement TargetLow { get; set; }
        public IElement TargetHigh { get; set; }
        public Bot()
        {
            MicroChips = new List<int>();
            History = new List<(int low, int high)>();
        }
        ...
        internal void GiveChip()
        {
            MicroChips.Sort();
            TargetLow.AddChip(MicroChips[0]);
            TargetHigh.AddChip(MicroChips[1]);
            History.Add((MicroChips[0], MicroChips[1]));
            if (MicroChips[0] == 17 && MicroChips[1] == 61)
                Console.WriteLine($"{Id} just compared chips {MicroChips[0]} and {MicroChips[1]}");
            MicroChips.Clear();
        }

        public bool IsActive()
        {
            return MicroChips.Count == 2;
        }
    }
}
