...
aoubshwq-pibbm-kcfygvcd-740[wnucy]";
            ...
            int sum = 0;
            var decipheredRoom = new List<(string, int)>();
            foreach (var room in rooms)
            {
                (string roomName, int sectorId, string checkSum) = Analyze(room);
                //Console.WriteLine(room);
                //Console.WriteLine($"{roomName} {sectorId} {checkSum}");
                if (IsCheckSumValid(roomName, checkSum))
                {
                    sum += sectorId;
                    var decipher = Decipher(roomName, sectorId);
                    if (decipher == "northpole object storage")
                    {
                        Console.WriteLine($"############## {sectorId} ##### {decipher}");
                    }
                    //Console.WriteLine($"{roomName} {decipher} {sectorId}");
                    decipheredRoom.Add((decipher, sectorId));
                }

            }
            decipheredRoom.Sort();
            //Console.WriteLine(string.Join("\n", decipheredRoom.Select(pair => $"{pair.Item1} {pair.Item2}")));
            Console.WriteLine(sum);
        }

        private static string Decipher(string roomName, int sectorId)
        {
            sectorId %= 26;
        ...
        private static char move(char c, int shift)
        {
            return (char)((c - 'a' + shift) % 26 + 'a');
        }

        private static (string roomName, int sectorId, string checkSum) Analyze(string room)
        {
            var regex = new Regex(@"^(?<roomName>[-a-z]+)-(?<sectorId>[0-9]+)\[(?<checksum>[a-z]+)]");
            var match = regex.Match(room);
            if (match.Success)
            {
                return (match.Groups["roomName"].Value, int.Parse(match.Groups["sectorId"].Value), match.Groups["checksum"].Value);
            }
            throw new ArgumentException($"{room} not valid roomname");
        }

        private static bool IsCheckSumValid(string room, string checkSum)
        {
            return checkSum == string.Join("", room.Where(c => c != '-').GroupBy(c => c).OrderByDescending(gr => gr.Count()).ThenBy(gr => gr.Key).Take(5).Select(gr => gr.Key));
        }
    }
}
