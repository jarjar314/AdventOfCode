...
xuabbxdwkutpsogcfea[tgetfqpgstsxrokcemk]cbftstsldgcqbxf[vwjejomptmifhdulc]ejeroshnazbwjjzofbe";
            ...
            Console.WriteLine(inputs.Count(SupportTLS));
            Console.WriteLine(inputs.Count(SupportSSL));
        }

        private static bool SupportSSL(string address)
        {
            ...
            for (int i = 0; i < address.Length - 3; i++)
            {
                ...
                else
                {
                    ...
                    if (address[i + 1] == '[' || address[i + 1] == ']') continue;
                    if (address[i] == address[i + 2])
                    {
                        if (insideBracket)
                        {
                            inside.Add(address[i].ToString() + address[i + 1]);
                        }
                        else
                            outside.Add(address[i + 1].ToString() + address[i]);
            ...
            inside.IntersectWith(outside);
            return inside.Any();
        }

        private static bool SupportTLS(string address)
        {
            bool valid = false;
            bool insideBracket = false;
