...
namespace AOC2021
{
    public class Day8
    {
        ...
        {
            var input = @"cg fadegbc ecfadb acdbeg abgfe dcegfb gcad bceag debca bgc | ceafbd gfedcb cabedf dbace
...
cbgaed fbagc cbfd bdaegf bdcag egdbfac afgce bcfadg baf bf | fb baf edgafb cbgda";
            ...
            {
                var elems = line.Split(new char[] { ' ', '|' }, StringSplitOptions.RemoveEmptyEntries).Select(c => sortString(c)).ToArray();
                count1478 += elems.Skip(10).Count(c => c.Length == 2 || c.Length == 3 || c.Length == 4 || c.Length == 7);
                var map = Identify(elems.Take(10).ToArray());
                var sum = Read(elems.Skip(10).Take(4).ToArray(), map);
                sumTotal += sum;
            }
        ...
        private static int Read(string[] vs, Dictionary<string, int> map)
        {
            var res = 0;
            foreach (var v in vs)
            {
        ...
        private static string sortString(string c)
        {
            return new string(c.OrderBy(ch => ch).ToArray());
        }

        private static Dictionary<string, int> Identify(string[] vs)
        {
            char a, b, c, d, e, f, g;
            var counting = new int[7];
            foreach (var digit in vs)
            {
            ...
            var sept = vs.Where(s => s.Length == 3).First();
            var un = vs.Where(s => s.Length == 2).First();
            var quatre = vs.Where(s => s.Length == 4).First();
            var huit = vs.Where(s => s.Length == 7).First();
            var hs7 = sept.ToHashSet();
            hs7.RemoveWhere(ch => un.Contains(ch));
            a = hs7.First();
            e = Find(counting, 4);
            b = Find(counting, 6);
            f = Find(counting, 9);
            var hs1 = un.ToHashSet();
            hs1.Remove(f);
            c = hs1.First();
            var hs4 = quatre.ToHashSet();
            hs4.Remove(b); hs4.Remove(c); hs4.Remove(f);
            d = hs4.First();
            g = (char)('a' + (0 + 1 + 2 + 3 + 4 + 5 + 6 - (a + b + c + d + e + f - 6 * 'a')));
            ...
            mapping[sortString(new string(zero))] = 0;
            mapping[sortString(new string(one))] = 1;
            mapping[sortString(new string(two))] = 2;
            mapping[sortString(new string(three))] = 3;
            mapping[sortString(new string(four))] = 4;
            mapping[sortString(new string(five))] = 5;
            mapping[sortString(new string(six))] = 6;
            mapping[sortString(new string(seven))] = 7;
            mapping[sortString(new string(eight))] = 8;
            mapping[sortString(new string(nine))] = 9;
            return mapping;
        }

        private static char Find(int[] counting, int v)
        {
            for (int i = 0; i < 7; i++)
            {
                if (counting[i] == v)
